...
impl BitAnd for PGA3D {
    ...
    fn bitand(self: PGA3D, b: PGA3D) -> PGA3D {
        ...
        let a = self;
        res[15]=(a[15]*b[15]);
		res[14]=-(a[14]*-1.0*b[15]-a[15]*b[14]);
		res[13]=-(a[13]*-1.0*b[15]-a[15]*b[13]);
		res[12]=-(a[12]*-1.0*b[15]-a[15]*b[12]);
		res[11]=-(a[11]*-1.0*b[15]-a[15]*b[11]);
		res[10]=1.0*(a[10]*b[15]+a[13]*-1.0*b[14]*-1.0-a[14]*-1.0*b[13]*-1.0+a[15]*b[10]);
		res[9]=1.0*(a[9]*b[15]+a[12]*-1.0*b[14]*-1.0-a[14]*-1.0*b[12]*-1.0+a[15]*b[9]);
		res[8]=1.0*(a[8]*b[15]+a[11]*-1.0*b[14]*-1.0-a[14]*-1.0*b[11]*-1.0+a[15]*b[8]);
		res[7]=1.0*(a[7]*b[15]+a[12]*-1.0*b[13]*-1.0-a[13]*-1.0*b[12]*-1.0+a[15]*b[7]);
		res[6]=1.0*(a[6]*b[15]-a[11]*-1.0*b[13]*-1.0+a[13]*-1.0*b[11]*-1.0+a[15]*b[6]);
		res[5]=1.0*(a[5]*b[15]+a[11]*-1.0*b[12]*-1.0-a[12]*-1.0*b[11]*-1.0+a[15]*b[5]);
		res[4]=1.0*(a[4]*b[15]-a[7]*b[14]*-1.0+a[9]*b[13]*-1.0-a[10]*b[12]*-1.0-a[12]*-1.0*b[10]+a[13]*-1.0*b[9]-a[14]*-1.0*b[7]+a[15]*b[4]);
		res[3]=1.0*(a[3]*b[15]-a[6]*b[14]*-1.0-a[8]*b[13]*-1.0+a[10]*b[11]*-1.0+a[11]*-1.0*b[10]-a[13]*-1.0*b[8]-a[14]*-1.0*b[6]+a[15]*b[3]);
		res[2]=1.0*(a[2]*b[15]-a[5]*b[14]*-1.0+a[8]*b[12]*-1.0-a[9]*b[11]*-1.0-a[11]*-1.0*b[9]+a[12]*-1.0*b[8]-a[14]*-1.0*b[5]+a[15]*b[2]);
		res[1]=1.0*(a[1]*b[15]+a[5]*b[13]*-1.0+a[6]*b[12]*-1.0+a[7]*b[11]*-1.0+a[11]*-1.0*b[7]+a[12]*-1.0*b[6]+a[13]*-1.0*b[5]+a[15]*b[1]);
		res[0]=1.0*(a[0]*b[15]+a[1]*b[14]*-1.0+a[2]*b[13]*-1.0+a[3]*b[12]*-1.0+a[4]*b[11]*-1.0+a[5]*b[10]+a[6]*b[9]+a[7]*b[8]+a[8]*b[7]+a[9]*b[6]+a[10]*b[5]-a[11]*-1.0*b[4]-a[12]*-1.0*b[3]-a[13]*-1.0*b[2]-a[14]*-1.0*b[1]+a[15]*b[0]);
        res
    }
}
...
impl Sub<PGA3D> for float_t {
    type Output = PGA3D;

    fn sub(self: float_t, b: PGA3D) -> PGA3D {
        let mut res = PGA3D::zero();
        let a = self;
        res[0] = a-b[0];
        res[1] = -b[1];
...
impl Sub<float_t> for PGA3D {
    type Output = PGA3D;

    fn sub(self: PGA3D, b: float_t) -> PGA3D {
        let mut res = PGA3D::zero();
        let a = self;
        res[0] = a[0]-b;
        res[1] = a[1];
